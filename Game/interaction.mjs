...
import { createLine, createArrow, moveArrow } from "./environment.mjs";
  ...
  function interact() {
    ...
    if (grabbed) {
      if (shadowObject) {
        ...
        torque
          .copy(relativeGrabPoint)
          .cross(globalForce)
          .multiplyScalar(scale_torque);
        physics.applyForceTorque(
          selectedObject.physicalBody,
          globalForce.multiplyScalar(scale_force),
          torque
        );
      } else if (hitObject) {
        ...
        localHitPoint.copy(
          hitPoint
            .clone()
            .applyMatrix4(shadowObject.matrixWorld.clone().invert())
        );
